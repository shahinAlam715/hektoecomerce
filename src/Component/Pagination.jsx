...
const Pagination = ({pageNumber, paginate, currentpage, prev, next}) => {
  return (
    <>
    ...
    <li onClick={()=>paginate(item)}>
      <a  className={`${currentpage == i + 1 ? "flex items-center justify-center px-3 h-8 leading-tight text-[#fff] bg-[#262626] border border-gray-300" : "flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300"}`}>{item + 1}</a>
    </li>
    ))}
    <li onClick={next}>
